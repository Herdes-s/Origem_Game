...
import { findNearestAvailableNode, getActiveYield, type ResourceNodeState } from "../../../entities/items/world/resourceNode";
...
type Args = {
  ...
  onNearNodeChange: (node: ResourceNodeState | null, actionLabel: string | null) => void;
};
  ...
  useEffect(() => {
    const loop = (timestamp: number) => {
      ...
      if (gameStateRef.current === "playing") {
        ...
        // filtrar. "Disponível" já considera ferramenta necessária (um
        // nó de picareta sem o player ter uma não conta).
        ...
        if (nearestNodeId !== lastNearNodeIdRef.current) {
          ...
          // Resolve o rótulo do botão AQUI (dentro do loop, onde ler ref
          // é seguro) — não em GamePage/render, que é proibido ler ref
          // durante o render. GamePage só guarda o texto já pronto.
          const activeYield = nearestNode
            ? getActiveYield(nearestNode, totalPlayedMsRef.current, inventoryRef.current)
            : null;
          onNearNodeChange(nearestNode, activeYield?.yieldConfig.actionLabel ?? null);
        }
